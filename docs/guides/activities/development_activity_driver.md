...
- [x] 将每个 V3 条款映射到具体代码文件（一条条款至少一个责任文件）。
- [x] 在 `docs/guides/drivers/backend_driver.md` 增加“本周开发焦点”滚动区块。
- [x] 增加“重构迁移状态表”（未开始/开发中/可联调/可验收）。
- [x] 将开发任务拆分为可独立提交的最小单元（避免大提交难回归）。

## 3. 验收标准

- [x] 每个开发任务都能追溯到 specs 条款与 codebase 文件节点。
- [x] 每个提交都包含代码与文档的同步更新，不出现“只改代码不改文档”。
- [x] 核心模块改动均可通过最小回归用例验证。
- [x] 开发节奏可持续推进，不依赖口头约定补全上下文。
