...
- [x] 为每个目标目录补充“必须暴露接口”清单。
- [x] 为每个迁移映射补充“迁移方式”（复制改造/直接重写/弃用）。
- [x] 在 `docs/codebase/backend` 中增加 V2->V3 文件对照链接。

## 3. 验收标准

- [x] 后端重构后的目录结构与本文蓝图一致，偏差有文档记录。
- [x] 每个旧模块都能映射到新的责任模块，不出现无主逻辑。
- [x] 模块依赖方向满足约束，不出现跨层反向依赖。
