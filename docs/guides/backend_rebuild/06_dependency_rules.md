...
4. 自动检查命令：`bash docs/tools/check_backend_dependencies.sh`。

## 2. 未来目标 TODO

- [x] 增加自动化依赖检查脚本（扫描 `import` 关系并输出违规项）。
- [x] 为每个分层补充“可对外暴露 API 白名单”。
- [x] 将依赖规则接入 PR 模板与发布检查清单。

## 3. 验收标准

- [x] 新增代码遵守允许依赖方向，无跨层反向依赖。
- [x] 评审时可快速定位违反规则的 import。
- [x] 依赖规则可独立指导新成员完成模块放置与引用决策。
