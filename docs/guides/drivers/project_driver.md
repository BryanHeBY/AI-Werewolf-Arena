...
- [x] 增加“文档-源码一致性”自动校验脚本（CI）。
- [x] 增加变更日志模板：每次代码变更同步文档摘要。
- [x] 增加按模块的风险/优先级看板。
- [x] 增加“白皮书条款 -> 代码文件”映射清单并持续维护。
- [x] 为后端重构新增“阶段进度总览面板”（P0~P4）。

## 3. 验收标准

- [x] `docs/codebase` 覆盖 backend/src 与 frontend/src 全部源码文件。
- [x] 所有节点文档包含“当前代码详细文档 / TODO / 验收标准”。
- [x] 新增源码文件后，文档镜像路径在同一提交内补齐。
- [x] 白皮书更新后，相关索引文档与 codebase 文档在同一提交内同步更新。
- [x] 后端重构可从本索引一键定位到“规范、任务、测试、回滚”四类文档。
