...
- [ ] `T01` 完成后端 P5 机制开发（狼队战术环 + 白天中断钩子）。
- [ ] `T02` 完成后端 P6 机制开发（警长竞选、警徽流转、1.5 票权）。
- [ ] `T03` 完成后端 P7 机制开发（记忆压缩、复杂规则冲突结算）。
- [ ] `T04` 将 P5~P7 对应测试补齐到 `backend/tests/v3/*` 并接入回归。
- [ ] `T05` 完成引擎开关、依赖边界规则与本地阻断能力。

## 3. 验收标准（任务映射）

- [ ] `A01`（对应: `T01`） P5 功能在回放测试中稳定通过且不破坏现有 MVP 用例。
- [ ] `A02`（对应: `T02`） P6 票权与竞选规则在自动化测试中可复现。
- [ ] `A03`（对应: `T03`） P7 压缩策略在 12 人局第 3 天后仍保持流程稳定。
- [ ] `A04`（对应: `T04`） 新增测试均纳入 `test:quick/test:full` 并通过。
- [ ] `A05`（对应: `T05`） 本地预检命令可自动阻断依赖逆向引用与关键回归失败。
