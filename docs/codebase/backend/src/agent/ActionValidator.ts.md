# backend/src/agent/ActionValidator.ts

## 1. 当前代码详细文档

- 源码路径：`backend/src/agent/ActionValidator.ts`
- 当前行数：`487`
- 文件职责：动作合法性校验与 fallback 策略。
- 对外暴露符号（来自当前代码扫描）：
- `12:export interface ValidationResult {`
- `18:export interface FallbackOptions {`
- `36:export class ActionValidator {`
- 内容简介：
