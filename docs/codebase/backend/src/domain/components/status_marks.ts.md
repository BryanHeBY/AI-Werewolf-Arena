# backend/src/domain/components/status_marks.ts

## 1. 当前代码详细文档

- 源码路径：`backend/src/domain/components/status_marks.ts`
- 文件类型：`ts`
- 当前行数：`50`
...
### 对外暴露类型/接口/函数
- `3:export class StatusMarksComponent implements PromptRenderable {`
- `48:export function createStatusMarksComponent(): StatusMarksComponent {`
