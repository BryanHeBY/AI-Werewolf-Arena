# backend/src/domain/components/camp.ts

## 1. 当前代码详细文档

- 源码路径：`backend/src/domain/components/camp.ts`
- 文件类型：`ts`
- 当前行数：`20`
- 文件定位：源码镜像文档节点。
...
### 对外暴露类型/接口/函数
- `3:export interface CampComponent extends PromptRenderable {`
- `7:export function createCampComponent(camp: Camp): CampComponent {`
