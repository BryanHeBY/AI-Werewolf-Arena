# backend/src/domain/components/alive.ts

## 1. 当前代码详细文档

- 源码路径：`backend/src/domain/components/alive.ts`
- 文件类型：`ts`
- 当前行数：`14`
...
### 对外暴露类型/接口/函数
- `3:export interface AliveComponent extends PromptRenderable {`
- `7:export function createAliveComponent(alive: boolean = true): AliveComponent {`
