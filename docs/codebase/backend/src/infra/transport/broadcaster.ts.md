# backend/src/infra/transport/broadcaster.ts

## 1. 当前代码详细文档

- 源码路径：`backend/src/infra/transport/broadcaster.ts`
- 文件类型：`ts`
- 当前行数：`41`
- 文件定位：V3 消息广播与传输适配实现文件。
...
### 对外暴露类型/接口/函数
- `3:export interface RealtimeGameEvent {`
- `9:export class Broadcaster {`
