# backend/src/gateway/action_validator.ts

## 1. 当前代码详细文档

- 源码路径：`backend/src/gateway/action_validator.ts`
- 文件类型：`ts`
- 当前行数：`156`
...
### 对外暴露类型/接口/函数
- `17:export interface ValidationContext {`
- `24:export class ActionValidator {`
...
- [x] 补齐 Function Calling schema 与错误回弹策略。
- [x] 补齐函数级输入/输出/副作用说明。
