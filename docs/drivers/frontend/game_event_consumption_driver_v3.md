...
> 状态：延后。当前前端已切换为离线 React 复盘播放器，不再保留 Vue/Socket 实现；本驱动仅保留为未来重新引入在线观战时的需求清单。

## 任务

- [ ] `FG01` 建立正式 V3 前端 store，统一消费 `gameEvent`
- [ ] `FG02` 建立正式 V3 Socket 适配层，只保留 `register` + `gameEvent`
- [ ] `FG03` 将 React 在线页面与主组件切到 V3 store
...
- [ ] `FV06` `bun test`、`tsc`、`vite build`、`playwright test` 通过

## 验收证据

1. 前端主入口：`frontend/src/App.tsx`
2. 未来正式 store：待在线观战范围恢复后设计
3. 未来正式网络层：待在线观战范围恢复后设计
