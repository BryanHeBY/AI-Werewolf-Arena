...
2. `docs/codebase` 已重建并覆盖当前 V3 单栈源码目录。
3. 当前主线进入“白皮书全角色与高级机制扩展”阶段。

## 2. 未来目标 TODO

- [x] 为 `guides/` 和 `references/` 增加自动化目录索引生成脚本。
- [x] 增加“文档阅读画像”：按角色（后端/前端/测试）给出最短路径。
- [x] 建立 docs 链接校验与章节校验 CI。
- [x] 增加“规范条款 -> 代码目录”追踪矩阵。
- [x] 增加“后端重构推进看板”入口并动态展示阶段状态。

## 3. 验收标准

- [x] `docs/` 文件命名遵循统一规则：常规文档小写，`docs/codebase` 镜像文档与源码同名同大小写。
- [x] 目录结构稳定为 `guides/references/specs/codebase` 四层入口。
- [x] 所有文档保留三段结构并可被直接用于开发驱动。
- [x] 入口文档到子文档的跳转路径无断链。
- [x] 重构前置文档可直接驱动后端从 P0 推进到 P4，无需额外规划文档。
