...
1. 单回合允许多次工具交互，工具结果逐次回填；
2. `finish_turn` 由模型显式调用申请结束回合；
3. 是否允许结束由 `turn_constraints` 判定；`must_act` 仅用于历史兼容回退；
4. 约束实现拆分为两层：
   - 判定层：`backend/src/agents/llm/turn_constraints.ts`
   - 渲染层：在提示词模板中生成系统/用户提示文案。

### 动作校验解耦（V3）

工具动作校验已从 loop 编排中拆分为独立服务：

1. `ActionValidationService` 负责工具可用性与参数合法性统一判定；
2. `LlmActionProvider` 仅负责消息流编排与回合收束；
3. 角色机制与警长机制仍保留在各自目录，不在中心编排层新增角色分支硬编码。
