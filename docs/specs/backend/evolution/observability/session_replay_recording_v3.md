...
开发驱动：`docs/drivers/backend/evolution/observability/session_replay_recording_v3_driver.md`
...
1. 记录目录固定在 `backend/data/records/`（后端数据目录）下。
2. 每局按 `session_id` 创建独立子目录：`backend/data/records/<session_id>/`。
...
### 2.4 `phase_windows.json`

用途：提供严格的阶段/子阶段窗口索引，用于 timeline 分段查询。

建议结构：

- `session_id: string`
- `windows: Array<{`
  - `phase_id: string`（建议：`d${day}-${phase}`）
  - `day: number`
  - `phase: string`
  - `start_seq: number`
  - `end_seq: number`
  - `stages: Array<{ stage:string; start_seq:number; end_seq:number }>`
`}>`

### 2.5 `timeline_index.json`

用途：提供快速统计与分页边界，降低查询开销。

建议结构：

- `session_id: string`
- `public: { min_seq:number; max_seq:number; count:number }`
- `players: Record<string, { count:number }>`
- `phases: { count:number }`

### 2.6 `players/player_<id>.json`
...
1. `backend/src/observability/session_record_manager.ts`
   - 负责 session 生命周期（`startSession/endSession`）与 JSON 文件落盘。
   - 管理序号、缓存、最终 flush。
2. `backend/src/observability/types.ts`
   - 统一 `manifest/public_timeline/logic_ops/player_view` 类型定义。
3. `backend/src/observability/json_writer.ts`
