...
1. 注册表：`backend/src/game/mechanisms/shared/text_localization_registry.ts`
2. 角色翻译贡献：`backend/src/game/mechanisms/roles/text_localization.ts`
3. 女巫药剂翻译贡献：`backend/src/game/mechanisms/roles/witch/text_localization.ts`
4. 终局胜负翻译贡献：`backend/src/game/mechanisms/win_conditions/text_localization.ts`
...
1. 玩家广播：`backend/src/game/mechanisms/broadcast/agent_event_line_registry.ts`
2. 玩家广播聚合：`backend/src/game/engine/agent_broadcast_feed.ts`
3. live 渲染：`backend/src/game/mechanisms/script/event_render_registry.ts`
4. 角色/机制事件文案：
   - `backend/src/game/mechanisms/roles/wolf/event_presenters.ts`
   - `backend/src/game/mechanisms/roles/seer/event_presenters.ts`
   - `backend/src/game/mechanisms/roles/guard/event_presenters.ts`
   - `backend/src/game/mechanisms/roles/witch/event_presenters.ts`
   - `backend/src/game/mechanisms/last_words/event_presenters.ts`
   - `backend/src/game/mechanisms/sheriff/event_presenters.ts`
5. 回放落盘：`backend/src/runtime/run_llm_game.ts` / `backend/src/observability/*`
