...
## 你提出的5项
1. 对局配置：已支持（`games/*.json`，内部映射 `BoardConfig`）。
...
### 1) 对局/板子层配置（`GameConfig` + `BoardConfig`）
- 位置：
  - `backend/src/core/domain/model.ts`
  - `backend/src/runtime/scenarios/six_player_mvp.ts`
  - `backend/src/runtime/scenarios/twelve_player_standard.ts`
  - `backend/src/runtime/config/runtime_config.ts`
- 可配置项：
  - `boardSize`：玩家数量
...
### 2) 胜利条件
- 位置：
  - `backend/src/game/mechanisms/win_conditions/default_specs.ts`
  - `backend/src/game/mechanisms/registries/win_condition_registry.ts`
- 已支持：
  - `SlaughterCity`
  - `SlaughterSide`

### 3) 警长机制相关
- 位置：`backend/src/game/mechanisms/sheriff/sheriff_mechanism.ts`
- 已支持规则：
  - 警长票权：`1.5`（当前为常量）
  - 警长投票仅警下可投
  - 候选、退水、投票流程
...
### 4) 运行与调试配置（运行时配置）
- 主要位置：
  - `backend/src/runtime/run_llm_game.ts`
  - `backend/src/runtime/config/index.ts`
  - `backend/src/ai/integrations/llm/openai_client.ts`
  - `backend/src/observability/debug_summary_generator.ts`
- 代表项：
  - OpenAI：`OPENAI_BASE_URL` / `OPENAI_API_KEY` / `OPENAI_MODEL` / `OPENAI_TEMPERATURE` / `OPENAI_MAX_TOKENS` / `OPENAI_FORCE_JSON`
  - V3 运行：`GAME_CONFIG_NAME` / `GAME_CONFIGS_DIR` / `V3_LLM_MAX_DAYS` / `V3_LLM_MAX_RUNTIME_MS` / `V3_LLM_TIMEOUT_MS`
...
### A. 女巫首夜自救
- 当前状态：
  - `canSelfHeal` 在 `backend/src/game/mechanisms/roles/witch/profile.ts` 初始化为 `disabled`。
  - 校验在 `backend/src/game/mechanisms/roles/witch/validation_rules.ts`。
- 建议：
...
- 建议：
  - 保持默认值，但在 `games/*.json` 中固定为可显式配置项，避免行为隐式。
