...
1. **纯粹串行 (Strictly Serial)**：放弃任何形式的状态并发竞争。完全还原面杀规则，按法官（上帝）台本严格排队唤醒，一次只处理一个实体的意图。
...
### 2.2 绝对串行的白天工作流 (Strict Daytime Pipeline)
...
4. **放逐投票**：全员发言完毕后，串行唤醒存活玩家调用 `vote(target)`。
...
* **Phase 4: 狼队战术环 (Wolf Tactical Loop)**
  * *步骤 A (发言)*：随机打乱狼人顺序，按同一顺序执行**两轮**串行 `speak_to_wolves()` 战术交流。
  * *步骤 B (投票)*：维持该顺序，依次调用 `kill_vote()` 提交意图。
