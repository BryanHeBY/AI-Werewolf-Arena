...
* **Phase 4: 狼队战术环 (Wolf Tactical Loop)**
  * *步骤 A (发言)*：随机打乱狼人顺序，依次串行调用私有工具 `speak_to_wolves()` 交流战术。
...
4. 将当前 `docs/codebase/backend/src/*` 文档逐步升级为“实现对白皮书条款”的逐文件映射表。

## 3. 验收标准

1. 信息完整性：
   - [x] 本文覆盖输入架构参考中的全部机制与角色信息，无缺漏。

2. 架构一致性：
   - [x] V3 后端实现与本文定义的串行时序、ECS 组件模型、工具鉴权模型一致。

3. 可执行性：
   - [x] 研发可仅凭本文完成模块拆分、接口定义与阶段实现。

4. 可验证性：
   - [x] 每条关键规则（中断钩子、狼队战术环、同守同救、反向查验等）均可被自动化测试回归验证。
