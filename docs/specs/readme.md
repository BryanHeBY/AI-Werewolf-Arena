...
- [x] 基于后端架构白皮书产出 V3 的可执行配置 schema（角色、阶段、钩子、胜负条件）。
- [x] 为每个技能生成标准化“触发-目标-冲突-结算”模板。
- [x] 将角色条目映射到后端 ECS 组件设计表。
- [x] 将 MVP 文档中的 Checklist 映射到实际 issue/任务卡，并持续更新状态。

## 3. 验收标准

- [x] 后端架构白皮书与 MVP 实现要求已纳入开发入口并被团队统一遵循。
- [x] 技术与玩法文档覆盖 V3 方案所需全部规则信息。
- [x] 文档内容可直接驱动后端重构设计，不依赖口头补充。
- [x] 文档结构与术语在后续迭代中保持一致并可追踪更新。
