import { GameState, PlayerAction, EnvironmentInterface, GameConfig, Player, BroadcastEvent, BroadcastEventType, ActionType, PublicGameState, PublicPlayer, Faction, RoleType } from './types';
...
export class Environment implements EnvironmentInterface {
  ...
  getPublicGameState(): PublicGameState {
    const publicPlayers: PublicPlayer[] = this.gameState.players.map(player => ({
      id: player.id,
      name: player.name,
  ...
  getVisibleHistory(playerId: number): PlayerAction[] {
    ...
    for (const action of this.gameState.history) {
      const shouldShow = this.shouldShowAction(action, playerId, player.role.faction);
      if (shouldShow) {
  ...
  private shouldShowAction(action: PlayerAction, observerId: number, observerFaction: Faction): boolean {
    ...
    if (action.actionType === ActionType.Speak || action.actionType === ActionType.Vote) {
      return true;
    }

    if (observerFaction === Faction.Wolf && action.roleType === RoleType.Wolf && action.actionType === ActionType.Kill) {
      return true;
    }
