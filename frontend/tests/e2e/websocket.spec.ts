...
// Skip WebSocket tests since we're using Mock Engine
test.describe.skip("AI Werewolf Arena - WebSocket Integration Tests", () => {
  test("frontend loads with WebSocket connection", async ({ page }) => {
    ...
    try {
      ...
      console.log(
        "⚠️ WebSocket connection status not visible or not implemented",
      );
    }
  });
  ...
  test("start game button triggers WebSocket connection", async ({ page }) => {
    ...
    const playerElements = page.locator(
      '.player-card, [data-testid*="player"]',
    );
  ...
  test("check backend API status", async ({ page }) => {
    ...
    try {
      const response = await page.request.get(
        "http://localhost:3344/api/status",
      );
  ...
  test("test game start API", async ({ page }) => {
    ...
    try {
      const response = await page.request.get(
        "http://localhost:3344/api/start-game",
      );
      expect(response.status()).toBe(200);
      ...
      console.log(
        `✅ Game start API works, created ${data.players.length} players`,
      );
